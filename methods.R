...
compare_graphs_with_formula <- function(graph_1, graph_2, names) {
  ...
  graph_1 <- `graph_list_w-plots_328`[[1]]
  graph_2 <- `graph_list_w-plots_328`[[1]] #название одинаковые, но граф другой, не сработало в предыдущем шаге
  graph_3 <- `graph_list_w-plots_328`[[1]]
...
# Узлы обоих графов объединяются в одну таблицу данных, 
  ...
  nodes <- nodes_1 %>%
    ...
    mutate(graph = ifelse(graph == 1, names[1], names[2])) %>% # names?
...
################################ метод 2 ########################
as_g1 <- as.igraph(graph_1)
as_g2 <- as.igraph(graph_2)

sg1 <- bootnet(matrix_1, function(i, ...) range(threshold(i)), R = 1000)

# boot_results <- numeric(1000)
# threshold_results <- numeric(1000)  # Для хранения результатов оценки диапазона порогов
# samples <- matrix(NA, nrow = 1000, ncol = 2)  # Создание матрицы для хранения выборок
# 
# for (i in 1:1000) {
#   # Генерация случайной выборки 
#   sample_indices <- sample(1:vcount(as_g1), replace = TRUE)
#   samples[i,] <- range(threshold(as_g1[sample_indices]))
#   
#   # Оценка диапазона порогов
#   boot_results[i] <- samples[i, 1]
#   threshold_results[i] <- samples[i, 2]
# }

sg2 <- bootnet(graph_2, function(i, ...) range(threshold(i)), R = 1000)
