...
find_file <- function(string) {
  ...
  file_names <- c()
  nodes_filenames <- list.files(path = "C:/Users/admin/Desktop/AA_networks-main/AA_networks-main/NaPi2b_AA/str_data/")
  nodes_filename <- nodes_filenames[str_detect(nodes_filenames, string)]
  file_names[1] <- str_c("C:/Users/admin/Desktop/AA_networks-main/AA_networks-main/NaPi2b_AA/str_data/", nodes_filename, sep = "")
  
  edges_filenames <- list.files(path = "C:/Users/admin/Desktop/AA_networks-main/AA_networks-main/NaPi2b_AA/aminoacids_interactions/")
  edges_filename <- edges_filenames[str_detect(edges_filenames, string)]
  file_names[2] <- str_c("C:/Users/admin/Desktop/AA_networks-main/AA_networks-main/NaPi2b_AA/aminoacids_interactions/", edges_filename, sep = "")
  
  return(file_names)
  
}
#result <- find_file("charmm-gui-1890760379.csv") #328
#result <- find_file("charmm-gui-3887718537.csv") #303
result <- find_file("charmm-gui-1717818438.csv") #no
nodes <-read_csv(result[1]) %>%
  dplyr::mutate(resid = str_c(resno, resid, sep = "_"))
names(nodes)[1:2] <- c('id', 'label')
edges <-read_csv(result[2]) %>%
  filter(persent_intractions != 0)%>%
  dplyr::mutate(five_aa = ifelse(abs(number.x - number.y) >= 5, 2, 1))
edges <- select(edges, 1, 2, 3,10)
...
get_network_parameters <- function(graph_list) {
  ...
  names(graph_graphs) <- rep(parameters, each=2)
  names(graph_graphs) <- paste(names(graph_graphs), c("_kk", "_xy"), sep = "")
...
compare_node_parameters <- function(parameter_list) {
  ...
  for (i in 1:length(node_parameters_names)) {
    ...
    pwc <- parameter_table_pre_plot %>%
      ...
      filter(group1 == "control") # ONLY COMPARISON WITH DEFAULT GRAPH (NO WEIGHT)
  ...
  names(comparison_node_parameters) <- rep(node_parameters_names, each = 3)
  names(comparison_node_parameters) <- paste(names(comparison_node_parameters), c("_kruskal", "_wilcox_pwc", "_rainplot"), sep = "")
...
graph_with_formula(nodes, edges, formula_coef, name)



 

##центр и промежут узлы
hb_1 <- graph_1 %>%
  activate(nodes) %>%
  as_tibble()

hub_threshold_1 <- quantile(hb_2$hub, 0.95)
betweenness_threshold_1 <- quantile(hb_2$betweenness, 0.95)

filtered_data_betweenness_1 <- subset(hb_1, betweenness >= betweenness_threshold_1)
filtered_data_hub_1 <- subset(hb_1, hub >= hub_threshold_1)
#
hb_2 <- graph_2 %>%
  activate(nodes) %>%
  as_tibble()

hub_threshold_2 <- quantile(hb_2$hub, 0.95)
betweenness_threshold_2 <- quantile(hb_2$betweenness, 0.95)

filtered_data_betweenness_2 <- subset(hb_2, betweenness >= betweenness_threshold_2)
filtered_data_hub_2 <- subset(hb_2, hub >= hub_threshold_2)
#
hb_3 <- graph_3 %>%
  activate(nodes) %>%
  as_tibble()

hub_threshold_3 <- quantile(hb_3$hub, 0.95)
betweenness_threshold_3 <- quantile(hb_3$betweenness, 0.95)

filtered_data_betweenness_3 <- subset(hb_3, betweenness >= betweenness_threshold_3)
filtered_data_hub_3 <- subset(hb_3, hub >= hub_threshold_3)