...
nodes_no <-read_csv(result[1])
edges_no <-read_csv(result[2]) %>%
  filter(persent_intractions > 0)
nodes_no$new_column <- paste(nodes_no$resno, nodes_no$resid, sep = "_")

name <- c("328", "303", "no")
#############################################################
# 1st function - Создание графов без и по каждому параметру #
#############################################################
parameters <- c('control', 'hbonds', 'RMSF', 'conservative', 'charge', 'hydrophobicity')
...
get_networks(nodes = nodes_no, edges = edges_no, parameters = parameters)
  ...
  for (i in 1:length(parameters)) {
    ...
    if (i == 1) {
      ...
      graph_plot_c_b_h <- graph_list[[i]] %>%
        ...
        geom_node_text(aes(label = resno, size = hub), repel = T) +
      ...
      node_pre_parameters <- graph_list[[i]] %>%
        activate(edges) %>%
        left_join(node_info, by = c("from" = "resno")) %>%
        left_join(node_info, by = c("to" = "resno"), suffix = c("_from", "_to")) %>%
        mutate(new_weight = !!sym(str_c(parameters[i], "_from")) + !!sym(str_c(parameters[i], "_to"))) %>%
        mutate(new_weight = 2**((new_weight - min(new_weight) / diff(range(new_weight)))) * persent_intractions)
...
graph_with_formula <- function(nodes, edges, formula_coef, name) {
  ...
  node_pre_parameters <- graph %>%
    ...
    left_join(node_info, by = c("from" = "resno")) %>%
    left_join(node_info, by = c("to" = "resno"), suffix = c("_from", "_to")) %>%
    mutate(new_weight = scale(coef_from + coef_to, center = F) * persent_intractions) #* five_aa #что это
  
  node_parameters <- node_pre_parameters %>%
    activate(nodes) %>%
    left_join(node_info, by = "resno") %>%
    mutate(betweenness = centrality_betweenness(weights = new_weight),
           hub = centrality_hub(weights = new_weight),
           degree = centrality_degree(weights = new_weight)) %>%
    as_tibble()
...
graph_with_formula(nodes_no, edges_no, formula_coef, name)
