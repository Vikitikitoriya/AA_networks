...
find_file <- function(string) {
  ...
  file_names <- c()
  nodes_filenames <- list.files(path = "C:/Users/Данил/Documents/Downloads/AA_networks/NaPi2b_AA/str_data/")
  nodes_filename <- nodes_filenames[str_detect(nodes_filenames, string)]
  file_names[1] <- str_c("C:/Users/Данил/Documents/Downloads/AA_networks/NaPi2b_AA/str_data/", nodes_filename, sep = "")
  
  edges_filenames <- list.files(path = "C:/Users/Данил/Documents/Downloads/AA_networks/NaPi2b_AA/aminoacids_interactions/")
  edges_filename <- edges_filenames[str_detect(edges_filenames, string)]
  file_names[2] <- str_c("C:/Users/Данил/Documents/Downloads/AA_networks/NaPi2b_AA/aminoacids_interactions/", edges_filename, sep = "")
  
  return(file_names)
  
}
# result <- find_file("charmm-gui-1717818438.csv")
# result <- find_file("charmm-gui-1890760379.csv")
result <- find_file("charmm-gui-3887718537.csv")
nodes <-read_csv(result[1])
edges <-read_csv(result[2]) %>%
  filter(persent_intractions > 0) 
nodes$new_column <- paste(nodes$resno, nodes$resid, sep = "_")
